...
void print_bit_operation_bin(Expression expression, unsigned int result)
{
    ...
    */
   print_binary(expression.op1);
   printf("\n");
   print_binary(expression.op2);
...
void print_bit_operation_hex(Expression expression, unsigned int result)
{
    ...
    */
    printf("Hex:\n");
    printf("0x%x %c 0x%x = 0x%x \n", expression.op1, expression.operation, expression.op2, result);
}
...
unsigned int bit_operation(Expression expression)
{
    ...
    {
        return expression.op1 & expression.op2;
    }
    if(expression.operation == '|')
    {
        return expression.op1 | expression.op2;
...
int main()
{
    ...
    do
    {
        ...
        Expression expression = { .op1 = operand1, .op2 = operand2, .operation = operation }; // Students: Create an expression
